...
public class Main {
	...
	static String mode = "enc"; // encrypt or decrypt
	...
	static boolean isShift = true; // -alg shift? default value if no -alg specified
	static boolean isUnicode = false; // -alg unicode?
	...
	private static void encUnicode() throws IOException {
	...
	private static void encShift() throws IOException {
		StringBuilder output = new StringBuilder();
		String outputStr = null;
		...
		for (int i = 0; i < input.length(); i++) {
			...
			// encode all chars
			int intChar = input.charAt(i); // c == 99
			char c = (char) intChar;

			// check if intChar is a letter, and if not - skip this part (do not decode)
			Character tmpChar = input.charAt(i);
			String tempStringLetterCheck = tmpChar.toString();

			if (tempStringLetterCheck.matches("[a-zA-Z]+")) {
				// check if lower case character
				if (intChar >= 97 && intChar <= 122) {
					lowCase = true;
				}

				c = (char) (intChar + shift); // 99 + 35 = 134
	...
	private static void decUnicode() throws IOException {
	...
	private static void decShift() throws IOException {
		StringBuilder output = new StringBuilder();
		String outputStr = null;
		boolean lowCase = false;
		...
		for (int i = 0; i < input.length(); i++) {
			...
			if (tempStringLetterCheck.matches("[a-zA-Z]+")) {
				...
				System.out.printf("Char is %d, shift is %d", intChar, shift);
				System.out.println();
